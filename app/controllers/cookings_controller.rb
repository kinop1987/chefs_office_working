class CookingsController < ApplicationController
  ...
  before_action :set_up_cooking, only: %i[show edit update destroy]
  before_action :check_collect_order, only: %i[show edit update detroy]
  before_action :search_cookings, only: %i[index search]
  ...
  def edit_cooking_params
    params.require(:cooking).permit(:name, :text, images_attributes: %i[name _destroy id])
  end
